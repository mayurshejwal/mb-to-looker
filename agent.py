...
class ValidationRouter(BaseAgent):
    ...
    failed entities (fix B) instead of resetting cursors to 0 and re-doing everything.
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        if wants_retry:
            ...
            retry_cards = out.get("retry_card_ids")      # optional scoping from validator
            ...
            # fix B: rebuild queues to ONLY the failed targets, then reset their
            # cursors. If the validator didn't scope, fall back to full re-run.
            if retry_cards:
                full = ctx.session.state.get("card_queue", [])
                delta["card_queue"] = [c for c in full
                                       if str(c.get("id")) in {str(x) for x in retry_cards}]
                delta["card_queue__cursor"] = 0
            else:
                delta["card_queue__cursor"] = 0  # full re-run only if unscoped
            if retry_dash:
                full = ctx.session.state.get("dashboard_queue", [])
                delta["dashboard_queue"] = [d for d in full
                                            if str(d.get("id")) in {str(x) for x in retry_dash}]
